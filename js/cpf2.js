...
function validarCPF () {
    ...
    const cpf = formataCPF(CPF);
    console.log('Formatado ', CPF);
    console.log('Sem Formatação ', cpf);
    
    if (cpf.length !== 11) {
        mostraResultado('CPF deve ter 11 digitos.' , 'white');
        return;

    }

    if (verificaDigitosRepetidos(cpf)) {
        mostraResultado('CPF com digitos repetidos,Por Favor verificar!', 'white');
        return;
    }
    ...
    if (!digito1 || !digito2) {
        ...
        mostraResultado(`CPF inválido, verifique e Tente Novamente. - ${cpf}`, 'white');
        return;
    }

    mostraResultado('CPF Valido.', 'black');
